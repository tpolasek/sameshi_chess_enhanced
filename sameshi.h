...
int E(int s, int depth, int alpha, int beta, int from, int to, int piece, int captured, int *has_legal_move) {
    // s: side to move
    // d: depth remaining
    // a: alpha
    // be: beta
...
int S(int s, int depth, int alpha, int beta) {
            ...
            if (type == 1) {
                ...
                if (!pass) {
                    ...
                    for (int dx = -1; dx <= 1; dx += 2) {
                        ...
                        if (captured && captured != 7 && (captured > 0) != (s > 0)) {
                            alpha = E(s, depth, alpha, beta, from, to, piece, captured, &has_legal_move);
                            if (alpha >= beta) return beta;
                        }
                    }
                } else if (!b[from + fwd]) {
                    ...
                    int to = from + fwd;
                    alpha = E(s, depth, alpha, beta, from, to, piece, 0, &has_legal_move);
                    if (alpha >= beta) return beta;
                ...
                r(i, start_dir, end_dir) {
                    ...
                    while (1) {
                        ...
                        if (!pass) {
                            if (target) {
                                alpha = E(s, depth, alpha, beta, from, to, piece, target, &has_legal_move);
                                if (alpha >= beta) return beta;
                                break;
                            }
                        } else {
